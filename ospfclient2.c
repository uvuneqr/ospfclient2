...
int main(int argc, char **argv) {
    ...
    // char *src_ip_str = "192.168.5.107";
    // char *src_mask_str = "255.255.252.0"; 
    ...
    for (ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next) {
        ...
        if (family == AF_INET) {
            ...
            if(!strcmp(src_interface,ifa->ifa_name)){
                src_ip_str = (char *)malloc(strlen(host)*sizeof(char*));
                memcpy(src_ip_str,host,strlen(host));
                printf("ip  mask:%s\n",src_ip_str);
                ...
                src_mask_str = (char *)malloc(strlen(host)*sizeof(char*));
                src_mask_str=host;
                printf("ip  mask:%s  %s\n",src_ip_str,src_mask_str);
                break;
            }
        }
    }
